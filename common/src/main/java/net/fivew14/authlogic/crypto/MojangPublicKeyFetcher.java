 ...
 * Uses in-memory cache with periodic refresh.
 */
public class MojangPublicKeyFetcher {
    ...
    public static synchronized CompletableFuture<List<PublicKey>> getPublicKeys() {
        ...
        if (!cachedPublicKeys.isEmpty() && (now - lastFetchTime) < CACHE_DURATION_MS) {
            LOGGER.debug("Using cached Mojang public keys ({} keys)", cachedPublicKeys.size());
    ...
    private static List<PublicKey> fetchPublicKeysFromAPI() throws Exception {
        ...
        LOGGER.info("Successfully fetched {} Mojang public keys", keys.size());
        return keys;
    }
